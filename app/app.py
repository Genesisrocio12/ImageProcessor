...
def upload_files():
    ...
    # Determinar tipo de carga para lógica de descarga
...
def process_images():
    ...
    # Validar opciones
    if not options['background_removal'] and not options['resize']:
        return jsonify({'error': 'Debe activar al menos una opción de procesamiento'}), 400
    
    print(f" Procesando {len(metadata['files'])} imágenes con opciones: {options}")
    ...
    for file_info in metadata['files']:
        ...
        if result['success']:
            print(f" {result['original_name']} -> {result['final_size']//1024}KB")
        else:
            print(f" {result['original_name']}: {result['message']}")
...
def download_processed(session_id):
    ...
    # SIEMPRE descargar individual si solo hay 1 archivo exitoso
    ...
    # Descarga múltiple en ZIP - solo cuando hay más de 1 archivo
...
if __name__ == '__main__':
    ...
    if REMBG_AVAILABLE:
        print(" Eliminación de fondo: ACTIVADA")
    else:
        print(" Eliminación de fondo: SIMULADA (instalar rembg para función completa)")
    ...
    try:
        result = subprocess.run(['oxipng', '--version'], 
                              capture_output=True, text=True, timeout=5)
        if result.returncode == 0:
            print(" Optimización oxipng: ACTIVADA")
        else:
            print(" Optimización oxipng: NO DISPONIBLE")
    except:
        print(" Optimización oxipng: NO DISPONIBLE")
